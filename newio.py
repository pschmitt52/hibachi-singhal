...
def get_arguments():
    ...
    parser.add_argument('-n', '--noise_pct', type=int, 
            help='percentage of noise for noise evaluation default=75 ')
    ...
    parser.add_argument("-c", "--column_subset", type=int,
            help="random subset of attributes to process. " + 
                 "Must be <= number of columns")
    ...
    if(args.noise_pct == None):
        options['noise_pct'] = 75
    else:
        options['noise_pct'] = args.noise_pct
    ...
    if(args.column_subset == None):
        options['column_subset'] = 0
    else:
        options['column_subset'] = args.column_subset
...
###############################################################################
def get_subset(data, subset, seed=None):
    """ return data with a random number of attributes """
    if seed != None: np.random.seed(seed)
    df = pd.DataFrame(data, index=None)
    clist = list(df.columns)
    dlist = random.sample(clist, subset)
    dlist.sort()
    subdata = df[dlist].values
    subx = subdata.transpose()

    return subdata.tolist(), subx.tolist(), dlist
###############################################################################
def create_file(x,result,outfile,selected=None):
